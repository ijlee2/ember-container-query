...
function setupCustomAssertions(assert) {
    ...
    for (const [featureName, meetsFeature] of Object.entries(features)) {
      switch (meetsFeature) {
        ...
        case false: {
          assert
            .dom(`[data-test-feature="${featureName}"]`)
            .hasText('false', `The container doesn't meet the feature "${featureName}".`);
  ...
  assert.areDimensionsCorrect = (expectedWidth, expectedHeight) => {
    assert
      ...
      .hasText(`${expectedWidth} x ${expectedHeight}`, 'Width and height are correct.');
    ...
    if (expectedAspectRatio === Infinity) {
      assert.strictEqual(aspectRatio === expectedAspectRatio, true, 'Aspect ratio is correct.');
    } else {
      assert.strictEqual(
